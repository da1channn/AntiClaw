...
function DiffViewer() {
  const { diffPreview, activePipeline, requestDiff } = useAppStore();
  const [expanded, setExpanded] = useState(false);
    ...
    <div className="border border-gray-800/50 rounded-xl overflow-hidden">
      <button
        onClick={() => {
          ...
          if (!expanded && !diffPreview && activePipeline) {
            requestDiff(activePipeline.id);
          }
        }}
        className="w-full flex items-center gap-2 px-3 py-2.5 hover:bg-gray-800/30 transition-colors"
      >
        <span className="text-sm">&#128203;</span>
        <span className="text-xs font-medium flex-1 text-left">\u5DEE\u5206\u30D7\u30EC\u30D3\u30E5\u30FC</span>
        <svg
          className={`w-3.5 h-3.5 text-gray-500 transition-transform ${expanded ? "rotate-180" : ""}`}
          fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}
        >
      ...
      {expanded && (
        <div className="border-t border-gray-800/50 max-h-80 overflow-auto">
          {diffPreview ? (
            <pre className="px-3 py-2 text-[11px] font-mono leading-relaxed whitespace-pre overflow-x-auto">
              {diffPreview.split("\n").map((line, i) => (
                <div
                  key={i}
                  className={
            ...
            <p className="px-3 py-4 text-xs text-gray-500 text-center">\u8AAD\u307F\u8FBC\u307F\u4E2D...</p>
          )}
        </div>
      )}
    </div>
...
function CommitApproval() {
  const { activePipeline, approveCommit, rejectCommit } = useAppStore();
